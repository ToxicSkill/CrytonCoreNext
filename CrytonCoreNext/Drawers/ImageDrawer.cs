...
namespace CrytonCoreNext.Drawers
{
    public class ImageDrawer
    {
        ...
        public static TransformBlock<AIImage, AIImage> CreatePipeline(Action<AIImage> result)
        {
            ...
            var step4 = new TransformBlock<AIImage, AIImage>(DrawHistogram, dfBlockOptions);
        ...
        private static AIImage SetAutoColor(AIImage image)
        {
            ...
            NormalizeRGBSpace(image);
            return image;
        }

        private static void NormalizeLABSpace(AIImage image)
        {
        ...
        private static void NormalizeRGBSpace(AIImage image)
        {
            using var rgbColorMat = image.PipelineMat.Clone(); 
            var channels = Cv2.Split(rgbColorMat);
            Cv2.CreateCLAHE(2.0, new(8, 8)).Apply(channels[0], channels[0]);
            Cv2.CreateCLAHE(2.0, new(8, 8)).Apply(channels[1], channels[1]);
            Cv2.CreateCLAHE(2.0, new(8, 8)).Apply(channels[2], channels[2]);
