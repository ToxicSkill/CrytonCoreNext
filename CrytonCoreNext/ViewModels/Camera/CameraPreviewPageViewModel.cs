                ...
                while (CameraContext.Camera != null)
                {
                    ...
                    await System.Windows.Application.Current.Dispatcher.BeginInvoke(() =>
                    {
                        if (RunCamera)
                        {
                            ...
                            if (RunDetection)
                            {
                                CameraContext.Camera!.ImageSource = _yoloModelService.PredictAndDraw(CameraContext.Camera, _cameraService.GetLastCameraFrame(), ScoreThreshold);
                            }
                            else
                            {
                                CameraContext.Camera!.ImageSource = _cameraService.GetLastCameraFrame().ToWriteableBitmap();
                            }
                        }
                    }, DispatcherPriority);
                    _cancellationToken.Token.ThrowIfCancellationRequested();
